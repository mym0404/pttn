  ...
  specCmd
    ...
    .action(async (concept?: string) => {
      ...
      try {
        ...
        if (concept) {
          console.log(pc.blue(`Starting specification planning for: "${concept}"`));
        } else {
          console.log(pc.blue('Ready to begin comprehensive specification planning'));
        }

        console.log(pc.dim('🔄 Interactive specification planning is available through Claude Code...\n'));

        console.log(pc.yellow('📋 To use the Interactive Specification Planning System:'));
        console.log(pc.dim('   1. Open Claude Code in your project'));
        
        if (concept) {
          console.log(pc.dim(`   2. Type: ${pc.bold(`/spec-create ${concept}`)}`));
        } else {
          console.log(pc.dim(`   2. Type: ${pc.bold('/spec-create')} [concept name]`));
        }
        
        console.log(pc.dim('   3. Engage in deep collaborative planning with AI agents'));
        console.log(pc.dim('   4. Generate multiple comprehensive specification files\n'));

        console.log(pc.green('✨ Features available in Claude Code:'));
        console.log(pc.dim('   • Multi-agent collaboration (Research, Architecture, UX, Business, Security)'));
        console.log(pc.dim('   • Deep requirement analysis with "why-chain" questioning'));
        console.log(pc.dim('   • Multiple interconnected specification files'));
        console.log(pc.dim('   • Professional-grade, implementation-ready output'));
        console.log(pc.dim('   • Iterative refinement and validation\n'));

        console.log(pc.cyan('💡 Alternative: For simple single-file specifications, you can use:'));
        console.log(pc.dim(`   ${pc.bold('npx -y cc-self-refer spec create-simple')} to use the basic creation mode\n`));
        
        // Suggest creating a simple spec if they provided content via CLI
        if (concept) {
          console.log(pc.blue(`Ready to plan specifications for: "${concept}"`));
          console.log(pc.dim('Use the Claude Code command above for the full interactive experience.'));
        }

      } catch (error) {
        console.error(pc.red('\n❌ Error launching specification planning system:'), error);
    ...
    .action(
      async (
        ...
        try {
          ...
          console.log(pc.dim(`   ${pc.bold('/spec-create')} in Claude Code`));
        } catch (error) {
